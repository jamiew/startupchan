class Notifier < ActionMailer::Base
  ...
  def user_signup_notification(user)
    mail(:to => "jamie@jamiedubs.com", :subject => "Startupchan user signup") do |format|
       format.text { render :text => "New user! ##{user.id} login=#{user.login.inspect}, email=#{user.email.inspect}" }
